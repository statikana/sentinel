...
class Utility(SentinelCog):
    ...
    async def avatar(self, ctx: SentinelContext, user = commands.param(converter=discord.User, default=lambda ctx: ctx.author, displayed_default="<author>")):
        embed = ctx.embed(
            title=f"`{user}`'s Avatar",
            description="Showing Guild Avatar"
        )
    ...
    async def rtfm(self, ctx: SentinelContext, project: str, query: str, version: Optional[str] = "stable", lang="en"):
        if ctx.interaction:
        ...
        for result in results:
            if ((name := result.select_one("a")) and (href := result.select_one("a")) and (source := result.select_one("span"))):
                name = name.text
                source = source.text.strip().strip("()")
                formatted_results.append(RTFMMeta(name, ref_url + "/" + str(href["href"]), source))
    ...
    async def roles(self, ctx: SentinelContext, member: discord.Member = commands.param(converter=discord.Member, default=lambda ctx: ctx.author, displayed_default="<author>")):
        description = f"**Highest Role:** {member.top_role.mention}\n"
        description += f"-- **All Roles** [*Descending*] --\n" + '\n'.join(role.mention for role in sorted(member.roles, key=lambda role: role.position, reverse=True)[:-1][:20])
        embed = ctx.embed(
            title=f"Role Information: `{member}`",
            description=description
        )
        await ctx.send(embed=embed)
...
class AvatarGlobalView(SentinelView):
    ...
    async def guild_avatar(self, itx: discord.Interaction, button: discord.ui.Button):
        ...
        await itx.response.edit_message(
            embed=embed,
            view=AvatarGuildView(self.ctx, self.user)
        )
